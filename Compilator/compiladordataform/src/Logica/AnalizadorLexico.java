...
public class AnalizadorLexico implements AnalizadorLexicoConstants {
	...
	private static void grabarLexema(int n, String token, String lexema, int nL, int nC) {
		System.out.printf("%-35s%-20s%-10s%-10s\u005cn", n + "- Lexema:" + lexema, "\u0009Token: " + token,
				"\u0009Linea: " + nL, "\u0009Columna: " + nC);
	}
	...
	public static void validarDeclaracionEstilo(String componente, String estilo, String valor) {
		if (estilo.equals("width") | estilo.equals("height") | estilo.equals("Tama\u00f1oFuente")) {
			if (!valor.equals("Tama\u00f1o")) {
				reportarError("Se esperaba un Tama\u00f1o en px para definir la propiedad " + estilo
						+ " del componente " + componente);
			}
			;
		} else if (estilo.equals("EstiloColor")) {
			if (!valor.equals("Color")) {
				reportarError(
						"Se esperaba un Color Hexadecimal para definir el " + estilo + " del componente " + componente);
			}
			;
		} else if (estilo.equals("Fuente")) {
			if (!valor.equals("Fuente")) {
				reportarError(
						"Se esperaba una Fuente de letra para definir la " + estilo + " del componente " + componente);
			}
			;
		} else if (estilo.equals("EstiloFuente")) {
			if (!valor.equals("EstiloFuente")) {
				reportarError("Se esperaba un Estilo de fuente (bold, italic) para definir el " + estilo
						+ " del componente " + componente);
			}
			;
		}
		...
		if (simbolo != null) {
			...
			if (tipo.equals("Tabla")) {
				if (!(estilo.equals("width") || estilo.equals("height"))) {
					reportarError("el Componente " + token
							+ " es una tabla y no puede tener asigandos valores distintos a width o height");
					return;
				}
				;
			}
			...
			if (tipo.equals("Patron")) {
				reportarError("el Componente " + token
						+ "  es un Patron y no puede tener elementos de estilos asociados a el");
				return;
			}
			;
	...
	public static void main(String args[]) throws ParseException {
		...
		try {
			...
			System.out.println(
					"-------------------------------- Iniciando An\u00e1lisis L\u00e9xico para el archivo archivoPrueba ----------------------------");
	...
	static final public String Componente() throws ParseException {
		...
		if (jj_2_3(2)) {
			...
			case CAMPOTEXTO:
				...
				ST.put(tkn.image, new SymbolTable("Campo de Texto", tkn.image)); {
				if (true)
					return tkn.image;
			}
				break;
			default:
				...
				if (jj_2_4(2)) {
					...
					case COMBOBOX:
						...
						ST.put(tkn.image, new SymbolTable("ComboBox", tkn.image)); {
						if (true)
							return tkn.image;
					}
						break;
					case BOTON:
						...
						ST.put(tkn.image, new SymbolTable("Boton", tkn.image)); {
						if (true)
							return tkn.image;
					}
						break;
					case LABEL:
						...
						ST.put(tkn.image, new SymbolTable("Label", tkn.image)); {
						if (true)
							return tkn.image;
					}
						break;
					case PATRON:
						...
						ST.put(tkn.image, new SymbolTable("Patron", tkn.image)); {
						if (true)
							return tkn.image;
					}
						break;
					case IDENTIFICADOR:
						tkn = jj_consume_token(IDENTIFICADOR);
						if (ST.get(tkn.image) == null)
							reportarError("la Variabe: " + tkn.image + " No esta declarada previamente"); {
						if (true)
							return tkn.image;
					}
						break;
	...
	static private boolean jj_3R_2() {
		if (jj_scan_token(IDENTIFICADOR))
			return true;
		if (jj_scan_token(SIMBOLOASIGNACION))
			return true;
		return false;
	}
	...
	static private boolean jj_3R_6() {
		if (jj_scan_token(SEPARADOR))
			return true;
		return false;
	}

	static private boolean jj_3R_13() {
		if (jj_scan_token(PATRON))
			return true;
		return false;
	}

	static private boolean jj_3R_11() {
		if (jj_scan_token(BOTON))
			return true;
		return false;
	}
	...
	static private boolean jj_3R_10() {
		if (jj_scan_token(COMBOBOX))
			return true;
		return false;
	}

	static private boolean jj_3R_9() {
		if (jj_scan_token(CAMPOTEXTO))
			return true;
		return false;
	}
	...
	private static void jj_la1_init_0() {
		jj_la1_0 = new int[] { 0x3f040, 0x3f040, 0x3f040, 0x3f040, 0x1000, 0x40, 0x3f040, 0x80000, 0x4000, 0x3b000,
				0x1000, 0x40, 0x3f040, 0x0, 0x80000, 0xf8000000, 0x4000780, };
	}

	private static void jj_la1_init_1() {
		jj_la1_1 = new int[] { 0x400, 0x400, 0x400, 0x400, 0x0, 0x0, 0x400, 0x0, 0x0, 0x0, 0x0, 0x0, 0x400, 0x8, 0x0,
				0x1, 0x4, };
	}
